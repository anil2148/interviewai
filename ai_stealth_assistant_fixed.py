...
#  ── LATEST FIXES ────────────────────────────────────────────────────────────
#   ✅ NO FOCUS STEAL: Qt.WindowDoesNotAcceptFocus + WA_ShowWithoutActivating
...
if HAS_PYQT5:
    ...
    class SettingsPanel(QWidget):
        ...
        def _on_whisper_change(self, s: str) -> None:
            global _whisper_model
            SETTINGS["whisper_model"] = s; save_settings(SETTINGS); _whisper_model = None
    ...
    class StealthOverlay(QWidget):
        ...
        def _init_ui(self) -> None:
            self.setWindowFlags(
                ...
                | Qt.Tool
                | Qt.WindowDoesNotAcceptFocus   # ← never steals focus from other apps
            )
            self.setAttribute(Qt.WA_ShowWithoutActivating)   # show without stealing focus
            self.setAttribute(Qt.WA_TranslucentBackground)
            self.setAttribute(Qt.WA_AlwaysStackOnTop)
            self.setAttribute(Qt.WA_X11DoNotAcceptFocus, True)   # X11/Linux extra guard
            self.setStyleSheet("background: transparent;")
            # NoFocus on the window itself — input_box keeps StrongFocus internally
        ...
        def _build_sidebar(self) -> QWidget:
            ...
            hdr_lbl.setFont(QFont("Arial", 10, QFont.Bold))
            ...
            self.pins_list.setFocusPolicy(Qt.NoFocus)
        ...
        def _build_main_area(self) -> QWidget:
            ...
            self.prompt_text.setFocusPolicy(Qt.NoFocus)   # no focus steal on click
            ...
            ai_name.setFont(QFont("Arial", 10, QFont.Bold))
            ...
            self.response_text.setFocusPolicy(Qt.NoFocus)
            ...
            self.char_counter.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
            ...
            self.mic_btn.setToolTip("Listen (Ctrl+L) — fills box, press Enter to send")
            ...
            self.inline_stop_btn.setCursor(QCursor(Qt.PointingHandCursor))
            ...
            self.clr_btn.setCursor(QCursor(Qt.PointingHandCursor))
            ...
            legend = QLabel(
                "Ctrl+L = listen  │  Ctrl+R = retry  │  ⏹ = stop  │  Ctrl+M = mute  │  Ctrl+Shift+H = hide"
            )
        ...
        def _small_btn(self, text, bg, fg, fn) -> QPushButton:
            ...
            b.setFont(QFont("Arial", 9)); b.setCursor(QCursor(Qt.PointingHandCursor))
            b.setStyleSheet(f"""
        ...
        def mousePressEvent(self, e) -> None:
            ...
            # Do NOT call super() here — that would activate the window

        def mouseMoveEvent(self, e) -> None:
            if e.buttons() == Qt.LeftButton:
                self.move(e.globalPos() - self.drag_pos)
            # Do NOT call super() — keep the window passive
        ...
        def _start_thinking(self) -> None:
            ...
            self.stop_btn.show()   # titlebar stop (legacy, kept for signal path)
        ...
        def _stop_thinking(self) -> None:
            ...
            self.stop_btn.hide()
            # Physical inline stop button — disable and dim
        ...
        def _set_mic_active(self, active: bool) -> None:
            if active:
                self.mic_btn.setText("🔴"); self.mic_btn.setStyleSheet(
                    """QPushButton{background:rgba(170,18,18,230);color:#fff;
                    border-radius:10px;border:2px solid rgba(240,55,55,170);font-size:12px;}""")
        ...
        def _show_toast(self, message: str, color: str = "#00c864") -> None:
            ...
            toast.setFont(QFont("Arial", 11, QFont.Bold))
        ...
        def _open_settings(self) -> None:
            if self._settings_panel is None:
                ...
                # Ensure settings panel also never steals focus
                self._settings_panel.setWindowFlags(
                    Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint
                    | Qt.Tool | Qt.WindowDoesNotAcceptFocus)
                self._settings_panel.setAttribute(Qt.WA_ShowWithoutActivating)
        ...
        def keyPressEvent(self, e) -> None:
            ...
            shft = bool(e.modifiers() & Qt.ShiftModifier)

            if key == Qt.Key_L and ctrl:            self._on_mic();           e.accept(); return
            if key == Qt.Key_M and ctrl:            self._on_mute_toggle();   e.accept(); return
            if key == Qt.Key_Q and ctrl:            self._on_close();         e.accept(); return
            if key == Qt.Key_D and ctrl:            self._clear();            e.accept(); return
            if key == Qt.Key_E and ctrl:            self._on_export();        e.accept(); return
            if key == Qt.Key_R and ctrl and shft:   self._on_run_code();      e.accept(); return
            if key == Qt.Key_R and ctrl and not shft: self._on_retry();       e.accept(); return
            if key == Qt.Key_F and ctrl:            self._toggle_search();    e.accept(); return
            if key == Qt.Key_H and ctrl and shft:   self._panic();            e.accept(); return
        ...
        def _set_input(self, t: str) -> None:
            """Fill input box after voice transcription.
            NEVER steals focus from the user's current app — user must click
            the overlay or press Enter themselves to send."""
            self.input_box.blockSignals(True)
            self.input_box.setText(t)
            self.input_box.blockSignals(False)
            # Amber highlight for 3 s so user sees text is ready to review
            self.input_box.setStyleSheet("""
                QLineEdit { background:rgba(255,170,0,18); color:#e2e8f0;
                    border:none; padding:0;
                    selection-background-color:rgba(168,85,247,80); }
                QLineEdit::placeholder { color:#475569; }
            """)
        ...
        def _show(self) -> None:
            ...
            # Use OS-level raise without activation — never steals focus
            _raise_no_activate(self.winId())
...
def create_overlay_ui() -> "Optional[StealthOverlay]":
    ...
    if not HAS_PYQT5:
        print("[ERROR] PyQt5 not available"); return None
